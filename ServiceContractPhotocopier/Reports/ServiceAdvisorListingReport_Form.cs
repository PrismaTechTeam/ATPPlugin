using AutoCount.Authentication;
using AutoCount.Data;
using ServiceContractPhotocopier.Classes.BaseForms;

namespace ServiceContractPhotocopier.Reports
{
    // [MENU HIDDEN FOR CUSTOMER RELEASE - uncomment to show]
    // [AutoCount.PlugIn.MenuItem("Service Advisor Listing Report",
    // ParentMenuCaption = "Reports", MenuOrder = 100, ParentMenuOrder = 800)]
    [AutoCount.Application.SingleInstanceThreadForm(System.Windows.Forms.FormWindowState.Maximized, false)]
    public partial class ServiceAdvisorListingReport_Form : ScpPlaceholder_Form
    {
        protected override string FormCaption { get { return "Service Advisor Listing Report"; } }
        public ServiceAdvisorListingReport_Form() { InitializeComponent(); }
        public ServiceAdvisorListingReport_Form(UserSession userSession) : base(userSession) { InitializeComponent(); }
        public ServiceAdvisorListingReport_Form(DBSetting dbSetting) : base(dbSetting) { InitializeComponent(); }
    }
}
