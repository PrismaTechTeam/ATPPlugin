$(async function () {
  ...
  const techs = new Set(), statuses = new Set(), zones = new Set();
  rows.forEach(r => {
    if (r.cells[1]) techs.add(r.cells[1]);
    if (r.cells[3]) statuses.add(r.cells[3]);
    if (r.cells[8]) zones.add(r.cells[8]);
  });
  const fill = ($sel, set) =>
  ...
  const totalAvailable = payload.totalAvailable || rows.length;
  if (totalAvailable > rows.length) {
    $('#loading').text('Showing first ' + rows.length + ' of ' + totalAvailable + ' rows.').show();
  }
  ...
  $('#trackingFilter').on('keyup', function () {
    dt.column(9).search(this.value, false, false).draw();
  });

  window.resetAllFilters = function () {
    $('#techFilter, #statusFilter, #zoneFilter, #locationFilter, #trackingFilter').val('');
