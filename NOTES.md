...
- NOTE-TO-SELF (validation 3b, needs CUSTOMER confirmation): period-mode window is FORWARD from the billed month's anchor (day-23 July run shows 23/07-22/08) per the demo's literal words, but the July reading physically measures ~23/06-22/07 - if the customer means billing-in-arrears the window should be anchor-of-PREVIOUS-month .. anchor-1 (23/06-22/07). One-line change in the generate-loop period block if they flip.
